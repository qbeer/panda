...
void tut_ana_fast(int nevts = 0, TString prefix = "signal")
{
	...
	while (theAnalysis->GetEvent() && i++<nevts)
	{
		...
		theAnalysis->FillList(muplus,  "MuonAllPlus"  , pidalgo );
		theAnalysis->FillList(muminus, "MuonAllMinus" , pidalgo );
		theAnalysis->FillList(piplus,  "PionAllPlus"  , pidalgo );
		theAnalysis->FillList(piminus, "PionAllMinus" , pidalgo );
		...
		theAnalysis->FillList(piminus, "PionLooseMinus", pidalgo);
